...
function HomePage() {
  ...
  return (
    <main className="relative container mx-auto px-4 pb-16 min-h-screen">
      ...
      {/* Category filters */}
      <div className="sticky top-[56px] z-40 bg-white/90 backdrop-blur-md border-b border-rose-100 shadow-sm px-3 py-2 rounded-b-2xl md:static md:bg-transparent md:shadow-none md:border-0 md:px-0 md:py-0">
              ...
              className={`flex-shrink-0 px-4 py-2 text-sm md:text-base font-myanmar rounded-full border transition-all duration-500 shadow-sm
                ${
                  activeCategory === category
                    ? "bg-gradient-to-r from-red-500 to-rose-400 text-red-900 border-red-600 shadow-lg scale-105"
                    : "bg-white text-red-800 border-rose-300 hover:shadow-md hover:text-red-600 hover:-translate-y-1"
                }`}
            >
              {category}
            </button>
      ...
      <div className="relative z-10 space-y-10 md:space-y-16 mt-6">
        {Object.entries(filteredMenu).map(([section, items], secIdx) => (
          ...
          >
            {/* Section title */}
            <h2 className="text-xl md:text-3xl font-extrabold text-red-900 mb-6 md:mb-12 text-center">
              {section}
              <span className="block w-12 md:w-20 h-1 bg-gradient-to-r from-rose-400 to-red-600 mx-auto mt-2 rounded-full" />
            </h2>
            {/* Menu list */}
              ...
              {items.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 md:gap-4 w-2/3">
                    ...
                    />
                    <span className="text-base md:text-xl font-myanmar text-red-900">
                      {item.name}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 justify-end w-1/3">
                    {[item.price, item.price2, item.price3]
                      ...
                      .map((p, i) => (
                        <span
                          key={i}
                          className="px-3 py-1 text-xs md:text-sm rounded-full bg-gradient-to-r from-rose-400 to-red-500 text-white font-semibold shadow hover:shadow-md transition-transform hover:-translate-y-0.5"
                        >
                          {p}
                        </span>
                      ))}
...
export default function App() {
  ...
  return (
    <Router>
      <div className="flex flex-col min-h-screen bg-white text-red-900">
        {/* Header */}
        <header className="sticky top-0 z-50 bg-white shadow-md">
          <div className="container mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <nav className="hidden md:flex gap-6 font-medium">
              ...
              <Link to="/special" className="hover:text-red-600">
                Special
              </Link>
            </nav>
        ...
        {/* Mobile Nav */}
        {isMobileMenuOpen && (
          <div className="md:hidden fixed top-[56px] left-0 w-full bg-white/95 backdrop-blur-md shadow-lg z-50 border-b border-rose-100 rounded-b-xl">
            <nav className="flex flex-col px-6 py-4 gap-4 text-red-800 font-medium">
              <Link
                to="/"
        ...
        {/* Footer */}
        <footer className="bg-rose-50 text-red-900 py-8 mt-12">
          <div className="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="flex gap-4 text-xl text-red-700">
              <a href="#" aria-label="Facebook">👍</a>
              <a href="#" aria-label="Instagram">📷</a>
              <a href="#" aria-label="Twitter">🐦</a>
            </div>
          </div>
          <div className="mt-4 text-center text-xs md:text-sm text-red-700">
